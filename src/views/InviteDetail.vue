  ...
  <div class="home">
    <Layout type="REVENUE" isRevenue="true" :pageTitle="$t('pageTitle')">
      <BasiceLayout :title="$t('pageTitle') " class="revenue-layout">
        ...
        <el-table v-if="detailList !== 'NONE'" :data="detailList" align="left" empty-text="-" style="width: 100%">
          <!-- invited account -->
          <el-table-column prop="email" align='center' :label="$t('invite_account')">
          </el-table-column>

          <!-- register time -->
          <el-table-column prop="sign_up_time" align='center' :label="$t('register_time')">
          </el-table-column>

          <!-- invite bxc -->
          <el-table-column prop="bxc" align='center' :label="$t('invite_bxc')">
          </el-table-column>
        </el-table>
        <div class="pagination" v-if="numSize > 1">
          <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-size="numSize"
            layout="total, prev, pager, next" :total="deviceLength">
          </el-pagination>
        </div>
      </BasiceLayout>
    </Layout>
